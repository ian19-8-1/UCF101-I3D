...
class InceptionI3d(nn.Module):
    def __init__(self, num_classes=101, spatial_squeeze=True, name='inception_i3d', in_channels=3,
        ...
        seq = [
            ...
            ('avg_pool', nn.AvgPool3d(kernel_size=[2, 7, 7], stride=(1, 1, 1), padding=2)),
            ('dropout', nn.Dropout(dropout_keep_prob)),
        ]

        self.logits = Unit3D(in_channels=384 + 384 + 128 + 128, output_channels=self.num_classes,
                             kernel_shape=[1, 1, 1], padding=1, act=None, norm=None, use_bias=True)
...
if __name__ == "__main__":
    ...
    per_frame_logits = i3d(x)
    print(per_frame_logits.shape)

    per_frame_logits = F.interpolate(per_frame_logits, clip_len, mode="linear", align_corners=True)
    print(per_frame_logits.shape)
